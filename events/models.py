...
class EventSet(models.Model):
    ...
    def __unicode__(self):
        return "%i: %s @ %s" % (self.pk, self.event_type, self.alias)
    ...
    def data_dir(self):
        d = None
        if self.pk == None:  # temporay, not saved to database
            d = os.path.join(settings.MEDIA_ROOT, self.alias + "/data")
        else:
    ...
    def event_list(self):
        ...
        d = self.data_dir()
        if d:
            results = [int(x) for x in os.listdir(self.data_dir()) if x.isdigit()]
        return results

    def event_count(self):
        if self.num_events != None:
    ...
    def summary(self):
        ...
        if os.path.exists(summary_file):
            ...
            if not self.pk:  # only for tmp dirs
                ...
                for event_id in self.event_list():
                    ...
                    if data_info:
                        for name, filename in data_info.items():
                            if name == "mc" or name.startswith("channel"):
                                continue
